...
function updateMovement() {
    ...
    if(dash.active && dash.duration > 0) {
        ...
        if(dash.duration <= 0) {
            ...
            setTimeout(() => { dash.canDash = true; updateDashUI(); }, 550);
        }
        isMoving = true;
...
function updateCooldowns() { if(cooldowns.skill1 > 0) cooldowns.skill1--; if(cooldowns.skill2 > 0) cooldowns.skill2--; if(cooldowns.skill3 > 0) cooldowns.skill3--; updateCooldownUI(); }
function updateEffects() { let i = effects.length; while(i--) { const e = effects[i]; e.life--; if(e.type === "spark") { e.x += e.vx; e.y += e.vy; } if(e.life <= 0) effects.splice(i,1); } }

function updateDashUI() { const el = document.getElementById('dashReady'); if(el) { if(dash.active) el.innerText = "РЫВОК"; else if(dash.cooldown > 0) el.innerText = `${Math.ceil(dash.cooldown/6)/10}с`; else el.innerText = "ГОТОВ"; } }
function updateCooldownUI() { const s1 = document.getElementById('skill1Hud'), s2 = document.getElementById('skill2Hud'), s3 = document.getElementById('skill3Hud'); if(s1) s1.classList.toggle('cooldown-active', cooldowns.skill1 > 0); if(s2) s2.classList.toggle('cooldown-active', cooldowns.skill2 > 0); if(s3) s3.classList.toggle('cooldown-active', cooldowns.skill3 > 0); }
function updateHealthUI() { const el = document.getElementById('hpValue'); if(el) el.innerText = player.hp; }
function resetGame() { player.hp = player.maxHp; player.x = MAP_W/2; player.y = MAP_H/2; player.invincible = 0; dash.active = false; dash.duration = 0; dash.cooldown = 0; dash.canDash = true; dash.trailPositions = []; dash.afterImages = []; updateHealthUI(); updateDashUI(); updateCamera(); }
...
function drawCursorMarker() {
    ...
    const sx = cursorWorld.x - camera.x, sy = cursorWorld.y - camera.y;
    ctx.beginPath(); ctx.arc(sx, sy, 7, 0, Math.PI*2); ctx.strokeStyle = "#ffaa44"; ctx.lineWidth = 1.5; ctx.setLineDash([4,5]); ctx.stroke();
    ctx.beginPath(); ctx.arc(sx, sy, 2.5, 0, Math.PI*2); ctx.fillStyle = "#ffaa66aa"; ctx.fill(); ctx.setLineDash([]);
}
...
function drawDirectionLine() {
    ...
    ctx.beginPath(); ctx.moveTo(sx, sy); ctx.lineTo(tx, ty); ctx.strokeStyle = "#fffbd0aa"; ctx.lineWidth = 1.5; ctx.setLineDash([5,7]); ctx.stroke();
    ctx.setLineDash([]);
}
...
let isMenuOpen = false, gamePaused = false;
...
exitBtn.addEventListener('click', () => { if(confirm('Вы уверены, что хотите выйти из игры?')) { window.close(); window.location.href = "about:blank"; } });
...
// ========== ПОЛНОЭКРАННЫЙ РЕЖИМ ==========
function resizeCanvas() { const asp = SCREEN_W / SCREEN_H; let w = window.innerWidth, h = window.innerHeight; if(w / h > asp) { w = h * asp; } else { h = w / asp; } canvas.style.width = `${w}px`; canvas.style.height = `${h}px`; canvas.style.left = `${(window.innerWidth - w) / 2}px`; canvas.style.top = `${(window.innerHeight - h) / 2}px`; }
function toggleFullscreen() { if (!document.fullscreenElement) { document.documentElement.requestFullscreen().then(() => setTimeout(resizeCanvas, 100)); } else { document.exitFullscreen().then(() => setTimeout(resizeCanvas, 100)); } }
