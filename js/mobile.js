...
function updateJoystick(touchX, touchY) {
    ...
    if(move.left || move.right || move.up || move.down) {
        ...
        if(Math.abs(joystickVector.x) > 0.2 || Math.abs(joystickVector.y) > 0.2) player.angle = angle;
    }
}
...
function resetJoystick() {
    ...
    move.left = false; move.right = false; move.up = false; move.down = false;
}

joystickBase.addEventListener('touchstart', (e) => { e.preventDefault(); joystickActive = true; const pos = getTouchPos(e.touches[0]); updateJoystick(pos.x, pos.y); });
joystickBase.addEventListener('touchmove', (e) => { e.preventDefault(); if(!joystickActive) return; const pos = getTouchPos(e.touches[0]); updateJoystick(pos.x, pos.y); });
joystickBase.addEventListener('touchend', (e) => { e.preventDefault(); joystickActive = false; resetJoystick(); });
...
// HUD ДЛЯ МОБИЛЬНЫХ (обновление без отображения)
